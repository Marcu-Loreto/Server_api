...
def analisar_sentimento(texto: str, modelo_sentimento: str):
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[{"role": "system", "content": "Retorne JSON estrito."},
        ...
        return {"label": label, "confidence": conf, "emotions": [str(e) for e in emotions if str(e).strip()], "reason": reason}
    except Exception as e:
        return {"label": "neutro", "confidence": 0.0, "emotions": [], "reason": f"Falha na análise: {e}"}
...
_PT_STOPWORDS = {
    ...
    "meu","minha","meus","minhas","seu","sua","seus","suas","depois","antes",
    "este","esta","estes","estas","isso","isto","aquele","aquela","aqueles","aquelas",
    "lhe","lhes","ele","ela","eles","elas","você","vocês","nós","nosso","nossa","nossos","nossas"
}
...
def build_cooc_graph(token_sequences, *, window_k: int = -1, min_edge_weight: int = 1) -> nx.Graph:
    """
    ...
    to_drop = [(u, v) for u, v, d in G.edges(data=True) if d.get("weight", 0) < int(min_edge_weight)]
...
def render_graph_pyvis(G, highlight_target: str = None, height_px: int = 600, dark_mode: bool = False):
    """
    ...
    if not _GRAPH_AVAILABLE or G is None or len(G) == 0:
        return None, "Grafo indisponível (instale: pip install networkx pyvis) ou sem dados."

    bg = "#0f172a" if dark_mode else "#ffffff"
    fg = "#e5e7eb" if dark_mode else "#333333"
    net = Network(height=f"{height_px}px", width="100%", bgcolor=bg, font_color=fg, notebook=False, directed=False)
...
def show_grafo_modal():
    ...
    import base64
    data_url = "data:text/html;base64," + base64.b64encode(html.encode("utf-8")).decode()
    st.sidebar.warning("Sua versão do Streamlit não suporta modal. Abra em nova aba:")
    st.sidebar.markdown(f'<a href="{data_url}" target="_blank">🧭 Abrir grafo em nova aba</a>', unsafe_allow_html=True)
...
with st.sidebar.container():
    ...
    if _hist:
        ...
        _last = _hist[-1]
        st.caption(f"Mensagens analisadas: {len(_scores)} | Último: {_last.get('label','?')} ({int(float(_last.get('confidence',0.0))*100)}%)")
    else:
...
with graph_container:
    ...
    min_edge_weight = st.slider("Mín. coocorrências (aresta)", 1, 5, 1, help="Filtra arestas fracas")
    # Define a profundidade máxima para caminhos até o alvo (limita subgrafo)
    max_path_depth = st.slider("Profundidade máx. caminho", 1, 8, 4, help="Caminhos até a palavra alvo")
    # Alterna entre ver o grafo completo ou apenas caminhos que chegam ao alvo
    show_paths_only = st.toggle("Mostrar apenas caminhos até a palavra alvo", value=True)
    # Modo escuro apenas para o grafo (melhor contraste)
    graph_dark_mode = st.toggle("Modo escuro (grafo)", value=True, help="Apenas afeta o grafo interativo.")
...
if "sentiment_history" not in st.session_state:
    st.session_state["sentiment_history"] = []  # histórico de scores por mensagem do usuário# <- sequências de tokens por mensagem (para grafo)
...
if mensagem_usuario:
    ...
    try:
        idx_user = sum(1 for m in st.session_state.get("lista_mensagens", []) if m.get("role") == "user")
    except Exception:
    ...
    if _data:
        st.session_state["sentiment_history"].append({
            ...
            "score": _score_from_label(_data.get("label","neutro"), float(_data.get("confidence", 0.0)))
        })
...
with wc_container:
    ...
    if err:
        ...
        st.download_button("📥 Baixar PNG", data=buf, file_name="wordcloud.png", mime="image/png", use_container_width=True)

with graph_container:
    # Calcula grafo a partir das sequências do usuário (aplica filtro de aresta)
    G_full = build_word_graph(st.session_state.get("user_token_sequences", []), min_edge_weight=min_edge_weight)
    ...
    if not _GRAPH_AVAILABLE:
        ...
        target = st.selectbox("Palavra alvo (destacar/filtrar caminhos):", options=top_words[:200] if top_words else ["(vazio)"])
        ...
        if G_view is not None and len(G_view) > 0:
            html, gerr = render_graph_pyvis(G_view, highlight_target=target, height_px=520, dark_mode=graph_dark_mode)
